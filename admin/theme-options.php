...
// footerに読み込み
function tcd_custom_admin_foot() {
?>
    ...
    $("#tab-panel").on( "click", ".ajax_button", function() {
      ...
      $('#myOptionsForm').ajaxSubmit({
        ...
        success: function(){
          ...
          $('#saveMessage').append("<p><?php _e('Settings Saved Successfully','tcd-w'); ?></p>").show();
        },
        error: function() {
          $('#saving_data').hide();
          alert('<?php _e('Can not save data. Please try again', 'tcd-w');  ?>');
        },
        timeout: 10000
      });
      setTimeout(function() {
...
add_action('admin_footer', 'tcd_custom_admin_foot');
...
require get_template_directory() . '/admin/inc/course.php';
...
require get_template_directory() . '/admin/inc/page.php';
...
/**
 * BIRTH オプションTools デフォルト画像設定フィルター
 */
function birth_theme_options_tools_get_default_images_settings( $default_images_settings ) {
	// デフォルト画像設定
	return array(
/*
		array(
...
*/
		array(
			'media_filename' => 'birth_top-slider.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/top-slider.gif',
			'theme_option_keys' => array( "['index_slider'][0]['image']", "['index_slider'][1]['image']", "['index_slider'][2]['image']" )
		),
		array(
			'media_filename' => 'birth_image_392x280.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_392x280.gif',
			'theme_option_keys' => array( 'index_3box_image1', 'index_3box_image2', 'index_3box_image3' )
		),
		array(
			'media_filename' => 'birth_image_1180x380.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1180x380.gif',
			'theme_option_keys' => array( 'index_intro_image' )
		),
		array(
			'media_filename' => 'birth_image_1450x440.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x440.gif',
			'theme_option_keys' => array( 'index_wide_image', 'archive_course_image', 'archive_news_image', 'archive_faq_image', 'archive_blog_image' )
		),
		array(
			'media_filename' => 'birth_image_130x130.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_130x130.gif',
			'theme_option_keys' => array( 'footer_content_image1', 'footer_content_image2', 'footer_content_image3' )
		),
	);
}
...
/**
 * BIRTH オプションTools サンプルカテゴリー
 */
function birth_theme_options_tools_set_sample_categories( $dp_options ) {
	// 新規サイト以外、リセットチェックなしの場合は終了
	if ( ! theme_options_tools_is_new_site() && empty( $_POST['tcd-tools-reset-sample-categories'] ) )
		return $dp_options;

	// トップページ画像 メディアID
	$media_id_index = theme_options_tools_get_media_id( array(
		'media_filename' => 'birth_image_510x300.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_510x300.gif'
	), true );

	// アーカイブ画像 メディアID
	$media_id_archive = theme_options_tools_get_media_id( array(
		'media_filename' => 'birth_image_1450x440.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x440.gif'
	), true );
	...
	$sample_categories = array(
		array(
			'name' => __( 'Couse Category1', 'tcd-w' ),
			'slug' => 'course-cat1',
			'taxonomy' => 'course_category',
			// オプション保存のカスタムフィールド meta_key => meta_value
			'metas' => array(
				'color' => '#e0b2b5',
				'index_image' => $media_id_index,
				'index_desc' => __( 'Here is a short description to be displayed on the top page. Here is a short description to be displayed on the top page.', 'tcd-w' ),
				'sub_title' => __( 'Sub title', 'tcd-w' ),
				'archive_image' => $media_id_archive,
				'headline' => __( 'Catchphrase for category archive page.', 'tcd-w' ), 
			)
		),
		array(
			'name' => __( 'Couse Category2', 'tcd-w' ),
			'slug' => 'course-cat2',
			'taxonomy' => 'course_category',
			// オプション保存のカスタムフィールド meta_key => meta_value
			'metas' => array(
	...
	// テーマオプション設定 コースカテゴリー
	$term_ids = array();
	foreach( array( 'course-cat1', 'course-cat2' ) as $term_slugs ) {
		$term_exists = term_exists( $term_slugs, 'course_category' );
		if ( ! empty( $term_exists['term_id'] ) )
			$term_ids[] = (int) $term_exists['term_id'];
	}
	if ( $term_ids )
		$dp_options['archive_course_cat'] = implode( ',', $term_ids );
...
/**
 * BIRTH オプションTools サンプル記事 ---------------------------------------------------------------------------
 */
function birth_theme_options_tools_set_sample_posts( $dp_options ) {

	$media_id = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'birth_image_830x510.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_830x510.gif'
	), true );
	...
	// course
	$media_id_course_header_image = theme_options_tools_get_media_id( array(
		'media_filename' => 'birth_image_1180x360.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_1180x360.gif'
	), true );
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample course', 'tcd-w' ) . $i,
			'post_content' => '',
			'post_name' => 'sample-course' . $i,
			'post_status' => 'publish',
			'post_type' => 'course',
			'post_date' => $post_date_base . sprintf( '%02d', $i ),
			'thumbnail' => $media_id,
			'taxonomies' => array(
				'course_category' => $i <= 2 ? 'course-cat1' : 'course-cat2'
			),
			'metas' => array(
				'header_image' => $media_id_course_header_image,
				'catch' => __( 'Here is catchphrase', 'tcd-w' ),
				'desc' => __( 'Here is a description. Here is a description. Here is a description.', 'tcd-w' ) . "\n" . __( 'Here is a description. Here is a description. Here is a description.', 'tcd-w' ),
				'short_desc' => __( 'Here is a short description. Here is a short description.', 'tcd-w' )
			)
		);
	}
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample faq', 'tcd-w' ) . $i,
			'post_content' => str_repeat( __( 'Answer here.', 'tcd-w' ), 6 ) . "\n" . str_repeat( __( 'Answer here.', 'tcd-w' ), 6 ),
			'post_name' => 'sample-post' . $i,
			'post_status' => 'publish',
			'post_type' => 'faq',
			'post_date' => $post_date_base . sprintf( '%02d', $i )
...
/**
 * BIRTH オプションTools サンプルメニュー ------------------------------------------------------------------
 */
function birth_theme_options_tools_set_sample_menus( $dp_options ) {
		...
		if ( $menu_item_db_id && ! is_wp_error( $menu_item_db_id ) ) {
			...
			if ( ! empty( $menu_item['children'] ) && is_array( $menu_item['children'] ) ) {
				foreach( $menu_item['children'] as $child_menu_item ) {
					if ( empty( $child_menu_item['name'] ) || empty( $child_menu_item['url'] ) )
						continue;

					wp_update_nav_menu_item( $menu_id, 0, array(
						'menu-item-type' => 'custom',
						'menu-item-title' => $child_menu_item['name'],
						'menu-item-url' => $child_menu_item['url'],
...
/**
 * BIRTH オプションTools サンプルウィジェット -----------------------------------------------------------------
 */
function birth_theme_options_tools_set_sample_widgets( $dp_options ) {
 ...
 * BIRTH テーマ初期化フィルター ----------------------------------------------------------------------------
 */
add_filter( 'tcd_theme_options_tools-get_default_images_settings', 'birth_theme_options_tools_get_default_images_settings', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'birth_theme_options_tools_set_sample_categories', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'birth_theme_options_tools_set_sample_posts', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'birth_theme_options_tools_set_sample_menus', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'birth_theme_options_tools_set_sample_widgets', 10, 1 );

...
function theme_option_page_post_list() {
  ...
  $page_post_list = new WP_Query($args);
  if ($page_post_list->have_posts()) {
    $html = '';
    $html .= '<div id="related_post">' . "\n";
    $html .= '<h3 class="headline">' . $options['page_post_list_headline'] . "</h3>\n";
    $html .= '<ol class="clearfix">' . "\n";
    while ($page_post_list->have_posts()) : $page_post_list->the_post();
      $html .= '<li class="clearfix">' . "\n";
      if(has_post_thumbnail()) {
        $html .= '<a class="image" href="' . esc_url(get_the_permalink()) . '" title="' . esc_html(get_the_title()) . '">';
        $html .= get_the_post_thumbnail($page_post_list->ID, 'size4');
        $html .= "</a>\n";
      }

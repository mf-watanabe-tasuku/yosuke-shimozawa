...
function po_admin_print_scripts() {
	...
	if ( 'faq' === $post_type || 'course' === $post_type ) {
...
/**
 * Change post order of custom post type 'faq' and 'course'
 */
function change_post_order() {
	...
	if ( 'faq' !== $post_type && 'course' !== $post_type ) return;
...
function po_pre_get_posts( $query ) { 
  ...
  // Modify post order of faq and course
  //if ( is_admin() || ! $query->is_main_query() ) return;
  if ( is_admin() ) return;

  if ( $query->is_post_type_archive( 'faq' ) || $query->is_post_type_archive( 'course' ) ) { 
